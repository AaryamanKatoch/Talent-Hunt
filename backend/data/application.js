...
const createApplication = async (
  jobSeekerId,
    ...
    if(!jobSeekerId)
    throw { status: "400", error: "No jobSeeker Id exists" };
    if(typeof(jobSeekerId)!=="string")
    throw { status: "400", error: "Type of jobSeeker Id is not a string" };
    if(jobSeekerId.trim().length===0)
    throw { status: "400", error: "jobSeeker Id cannot be empty or all white spaces" };
    ...
    if(!resumeId)
    throw { status: "400", error: "No resume Id exists" };
    if(typeof(resumeId)!=="string")
    throw { status: "400", error: "Type of resume Id is not a string" };
    if(resumeId.trim().length===0)
    throw { status: "400", error: "resume Id cannot be empty or all white spaces" };
...
const getAllApplications = async () => {    
  ...
  const getApplicationById = async (applicationId) => {  
    ...
    throw { status: "400", error: "Application id cannot be empty or all white spaces" };
   ...
   const applicationbyid = await applicationsCollection.findOne({_id:ObjectId(applicationId)});
  ...
  const removeApplication = async (applicationId) => {
    ...
    if (!deletename || deletename === undefined) {
        throw { status: "400", error: `Could not delete with id of ${applicationId}` };
    }

    const deletedapplication = await applicationsCollection.deleteOne({_id: ObjectId(applicationId)});
    if (deletedapplication.deletedCount === 0) {
        throw { status: "400", error: `Could not delete application with id of ${applicationId}` };
    }
  ...
  const updateApplication = async (
    ...
    const updatedInfo = await applicationsCollection.updateOne({_id: ObjectId(id)},
